...
export function createQueue<Q>({
    ...
    function tryExecute() {
        EXECUTION_PROMISE = EXECUTION_PROMISE || executeQueue().catch(() => null).finally(() => {
    ...
    async function executeQueue() {
        ...
        while (QUEUE.length) {
            ...
            runningPromises.push(...tasks.map(({done}) => {
                const promise = done.catch(() => null).finally(() => {
